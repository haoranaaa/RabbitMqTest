...
public class DefaultLogModule implements LogModule {
    ...
    /**
     * entryParam 的 index 就是 key. 严格保证递增.
     *
     * @param entryParam
     */
    @Override
    public void write(EntryParam entryParam) {

        boolean success = false;
        try {
            lock.tryLock(3000, MILLISECONDS);
            entryParam.setIndex(getLastIndex() + 1);
            logDb.put(entryParam.getIndex().toString().getBytes(), JSON.toJSONBytes(entryParam));
            success = true;
            LOGGER.info("DefaultLogModule write rocksDB success, entryParam info : [{}]", entryParam);
        } catch (RocksDBException | InterruptedException e) {
            LOGGER.warn(e.getMessage());
        } finally {
            if (success) {
                updateLastIndex(entryParam.getIndex());
            }
            lock.unlock();
        }
    }
    ...
    @Override
    public EntryParam read(Long index) {
        try {
